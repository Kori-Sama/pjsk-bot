...
from app.database import delete_expired_teams, get_upcoming_teams, get_team_members, delete_team_by_id
from app.routes import send_group_message
...
async def check_team_start_times():
    ...
    end_time = (current_time + datetime.timedelta(minutes=5)).strftime("%Y-%m-%d %H:%M:%S")
    ...
    for team in teams:
        ...
        print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 准备发送通知: {at_message}")
        # 假设team中有group_id字段，如果没有，需要从其他地方获取
        if 'group_id' in team:
            success = await send_group_message(team['group_id'], at_message)
            if success:
                print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 成功发送通知到群 {team['group_id']}")
            else:
                print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 发送通知到群 {team['group_id']} 失败")
        else:
            print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 无法发送通知: 缺少群ID信息")
